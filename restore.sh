...
usage() {
    cat <<'USAGE'
Usage: ./restore.sh <dump-file> [options]

  <dump-file>    A pg_dump SQL file, optionally gzipped (.sql or .sql.gz).

Options:
  --yes          Do not ask for confirmation. For scripts and cron.
  --no-backup    Skip the safety dump of the current database. Only sensible
                 when the database is already empty or already broken.
...
HEADER=$($READ_DUMP "$DUMP_FILE" 2>/dev/null | head -n 50 || true)
...
if ! printf '%s' "$HEADER" | grep -q "PostgreSQL database dump"; then
    ...
    MAGIC=$(head -c 2 "$DUMP_FILE" | od -An -tx1 | tr -d ' \n' || true)
    case "$MAGIC" in
        1f8b)
...
if [ "$CHECK_ONLY" = "yes" ]; then
    ...
    echo "=========================================="
    echo ""
    echo "The file is a PostgreSQL $DUMP_PG_MAJOR dump and this server is"
...
if [ "$ASSUME_YES" != "yes" ]; then
    ...
    echo "    replaced by the contents of that file. The mail on disk is not"
    echo "    touched, but the database that indexes it is."
    echo ""
...
if $READ_DUMP "$DUMP_FILE" | docker compose exec -T postgres \
...
# ============================================
# Step 6: Bring it back up
# ============================================
echo "Step 6: Starting up"
echo "-------------------"
