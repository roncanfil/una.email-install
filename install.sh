...
if [ "$SMTP_SUBDOMAIN" = "$WEB_SUBDOMAIN" ]; then
    echo "ℹ️  Both services share $SMTP_SUBDOMAIN.$DOMAIN, so one A record covers"
...
email deliverability, because most mail servers will reject or flag emails from servers without
a valid PTR record.
...
Create your account, then go to **Settings** and create your first email address, which you'll
...
- **SPF**: Should show green, confirming this server is authorized to send
- **DKIM**: Should show green, confirming your signature verifies
- **DMARC**: Should show green, confirming your domain policy is honoured
- **Blacklists**: Should show green, meaning your IP is on no list
- **PTR Record**: Should show green, meaning reverse DNS resolves
...
$(verify_row "SPF" "dig TXT $DOMAIN +short | grep -c v=spf1" "expect: exactly 1, because a 2 breaks SPF for every sender on your domain")
